const container = document.querySelector(".tvShows")
...
const renderPost = async (term) => {
  let urlPost = "http://localhost:3000/tvshows?_sort=id&_order=desc"
  ...
  return { props: { tvShows: data } }
}

const displayPosts = (tvShows) => {
  container.innerHTML = ""
  ...
  tvShows.forEach((tvShow) => {
    ...
    postLink.href = `/newtvshow.html?id=${tvShow.id}`
    postTitle.innerHTML = `${tvShow.title}`
    postTitle.className = "post-title"

    const postBody = document.createElement("p")
    postBody.innerHTML = `${tvShow.body.slice(0, 50)}`
